/**
 * Supabase Storage REST API File Uploader
 * 
 * To use this, add the following to your root .env file:
 * VITE_SUPABASE_URL=https://your-project-id.supabase.co
 * VITE_SUPABASE_ANON_KEY=your-anon-public-key
 * VITE_SUPABASE_BUCKET=cars
 */
...
export async function uploadImageToSupabase(file) {
  ...
  if (!isConfigured) {
    console.warn("Supabase credentials not configured in .env. Falling back to local preview URL for testing.");
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(`Supabase upload failed: ${response.statusText}. Details: ${errText}`);
    }
